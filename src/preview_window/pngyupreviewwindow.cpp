...
PngyuPreviewWindow::PngyuPreviewWindow(QWidget *parent)
  ...
  connect(&m_execute_compress_thread, SIGNAL(compress_finished()), this,
          SLOT(compress_finished()));

  connect(ui->checkBox_show_original, SIGNAL(toggled(bool)), this,
          SLOT(show_original_toggled(bool)));

  // connecst background select buttons
  connect(ui->toolButton_background_checkerboard, SIGNAL(toggled(bool)), this,
          SLOT(background_select_button_pressed()));
  connect(ui->toolButton_background_black, SIGNAL(toggled(bool)), this,
          SLOT(background_select_button_pressed()));
  connect(ui->toolButton_background_white, SIGNAL(toggled(bool)), this,
          SLOT(background_select_button_pressed()));

  connect(ui->toolButton_zoomin, SIGNAL(clicked()), this,
          SLOT(zoomin_pushed()));
  connect(ui->toolButton_zoomout, SIGNAL(clicked()), this,
          SLOT(zoomout_pushed()));

  connect(ui->pushButton_original_size, SIGNAL(clicked()), this,
...
void PngyuPreviewWindow::background_select_button_pressed() {
  const bool is_checker_checked =
      ui->toolButton_background_checkerboard->isChecked();
  ...
  if (is_checker_checked && !is_black_checked && !is_white_checked) {
    ui->imageview->setBackgroundBrush(
        QBrush(QPixmap(":background/checkerboard.png")));
...
void PngyuPreviewWindow::zoomin_pushed() { ui->imageview->scale(1.2, 1.2); }
